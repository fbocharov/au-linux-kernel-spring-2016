...
#define MAX_IO_BUF_SIZE 2 * PAGE_SIZE /* Comes from VSD device max buffer size. */
    ...
    char __user *read_user_buf, size_t read_size, loff_t *fpos)
{
     ...
     * 1. Driver execute wait_event_interruptible
     * 2. Interrupt comes from nowhere and control-flow goes to
     *    vsd_dev_dma_op_complete_tsk_func
     * 3. It returns driver into schedulers queue
     * 4. Scheduler switching to driver and it frees kernel buffer
     * 5. Scheduler switching to device and it writes to freed buffer
     * 6. ????
     * 7. :(((((
     */
